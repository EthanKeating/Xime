    ...
    public boolean execute(CommandSender sender, String alias, String[] args) {
        ...
        for(Serverable serverable : plugin.getServerHandler().getServerList()) {
            if (!serverable.getPlayerList().isEmpty()) {
                profile.sendMessage(String.format("&8[&3Xime&8] &8[&a%s&8] &fPlayers: &8[&6%s&8/&6%s&8]", serverable, serverable.getPlayerList().size(), serverable.getMaxPlayers()));
            }
        }
        profile.sendMessage(String.format("&8[&3Xime&8] &fThere are &8[&6%s&8/&6%s&8] &fplayers on the network.", Bukkit.getOnlinePlayers().size(), Bukkit.getMaxPlayers()));
