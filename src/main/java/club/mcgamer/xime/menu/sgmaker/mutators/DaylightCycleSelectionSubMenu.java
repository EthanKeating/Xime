...
public class DaylightCycleSelectionSubMenu extends FastInv {

    public DaylightCycleSelectionSubMenu(FastInv previousMenu, Profile profile, SGMakerServerable serverable) {
        super(9, TextUtil.translate("Daylight Cycle"));
        ...
        for(Pair<String, Boolean> togglePairs : toggles) {
            ...
            setItem(index, new ItemBuilder(Material.STAINED_CLAY)
                            ...
                            .build(),
                    e -> {
                        serverable.announce("&6Daylight Cycle: &e" + toggleDisplay);
                        serverable.getGameSettings().setDayLightCycle(toggle);
                        previousMenu.open(profile.getPlayer());
                    });
            index += 2;
        }
