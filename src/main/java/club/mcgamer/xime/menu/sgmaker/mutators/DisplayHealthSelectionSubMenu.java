...
public class DisplayHealthSelectionSubMenu extends FastInv {

    public DisplayHealthSelectionSubMenu(FastInv previousMenu, Profile profile, SGMakerServerable serverable) {
        super(9, TextUtil.translate("Display Health"));
        ...
        for(Pair<String, Boolean> togglePairs : toggles) {
            ...
            setItem(index, new ItemBuilder(Material.STAINED_CLAY)
                            ...
                            .build(),
                    e -> {
                        serverable.announce("&6Display Health: &e" + toggleDisplay);
                        serverable.getGameSettings().setDisplayHealth(toggle);
                        previousMenu.open(profile.getPlayer());
                    });
            index += 2;
        }
