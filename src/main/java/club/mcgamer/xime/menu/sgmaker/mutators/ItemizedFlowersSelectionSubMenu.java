...
public class ItemizedFlowersSelectionSubMenu extends FastInv {

    public ItemizedFlowersSelectionSubMenu(FastInv previousMenu, Profile profile, SGMakerServerable serverable) {
        super(9, TextUtil.translate("Itemized Flowers"));
        ...
        for(Pair<String, Boolean> togglePairs : toggles) {
            ...
            setItem(index, new ItemBuilder(Material.STAINED_CLAY)
                            ...
                            .build(),
                    e -> {
                        serverable.announce("&6Itemized Flowers: &e" + toggleDisplay);
                        serverable.getGameSettings().setItemizedFlowers(toggle);
                        previousMenu.open(profile.getPlayer());
                    });
            index += 2;
        }
