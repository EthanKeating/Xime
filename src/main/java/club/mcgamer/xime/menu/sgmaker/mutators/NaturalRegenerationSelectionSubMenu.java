...
public class NaturalRegenerationSelectionSubMenu extends FastInv {

    public NaturalRegenerationSelectionSubMenu(FastInv previousMenu, Profile profile, SGMakerServerable serverable) {
        super(9, TextUtil.translate("Natural Regeration"));
        ...
        for(Pair<String, Boolean> togglePairs : toggles) {
            ...
            setItem(index, new ItemBuilder(Material.STAINED_CLAY)
                            ...
                            .build(),
                    e -> {
                        serverable.announce("&6Natural Regeration: &e" + toggleDisplay);
                        serverable.getGameSettings().setNaturalRegeneration(toggle);
                        previousMenu.open(profile.getPlayer());
                    });
            index += 2;
        }
