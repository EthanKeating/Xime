...
public class RandomizeNamesSelectionSubMenu extends FastInv {

    public RandomizeNamesSelectionSubMenu(FastInv previousMenu, Profile profile, SGMakerServerable serverable) {
        super(9, TextUtil.translate("Randomize Names"));
        ...
        for(Pair<String, Boolean> togglePairs : toggles) {
            ...
            setItem(index, new ItemBuilder(Material.STAINED_CLAY)
                            ...
                            .build(),
                    e -> {
                        serverable.announce("&6Randomized Names: &e" + toggleDisplay);
                        serverable.getGameSettings().setRandomizeNames(toggle);
                        previousMenu.open(profile.getPlayer());
                    });
            index += 2;
        }
